function htmlToElement(e){const t=document.createElement("template");return e=e.trim(),t.innerHTML=e,t.content.firstChild}function initPopover(e,t,n){const s=e.replace(window.location.origin,"");fetchData.then(({content:e})=>{const o=[...document.getElementsByClassName("internal-link")];o.filter(e=>e.dataset.src||e.dataset.idx&&t).forEach(t=>{var o,i=t.dataset.src.replace(/#.*\/*/,"");if(i===window.location.pathname)return;if(console.log(t.dataset.src),t.dataset.ctx){const n=e[i],s=`<div class="popover">
							<h3>${n.title}</h3>
							<p>${highlight(removeMarkdown(n.content),t.dataset.ctx)}...</p>
							<p class="meta">${new Date(n.lastmodified).toLocaleDateString()}</p>
						</div>`;o=htmlToElement(s)}else{const n=e[i.replace(/\/*$/g,"").replace(s,"")];if(n){let s=t.href.split("#");console.log(t.href);let e=removeMarkdown(n.content);if(s.length>1){let t=decodeURIComponent(s[1]).replace(/-/g," "),n=e.toLowerCase().indexOf("<b>"+t+"</b>");e=e.substring(n,e.length)}const i=`<div class="popover">
                                <h3>${n.title}</h3>
                                <p>${e.split(" ",20).join(" ")}...</p>
                                <p class="meta">${new Date(n.lastmodified).toLocaleDateString()}</p>
